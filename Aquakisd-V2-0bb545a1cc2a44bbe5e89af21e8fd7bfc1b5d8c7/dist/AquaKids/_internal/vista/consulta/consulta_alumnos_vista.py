...
class ConsultaAlumnosVista(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.es_modo_solicitudes = False 
        self.columnas_editables = [9] 
    ...
    def _create_table(self):
        # MODO: ID, Nombre, Edad, Tel, Fecha Nac, Programa, Horario, Clases Restantes, Obs, Estado
        table = QTableWidget(0, 10)
        headers = [
            'ID', 'Nombre Completo', 'Edad', 'Teléfono',
            'Fecha Nac.', 'Programa', 'Horario', 'Clases Restantes', 'Observaciones', 'Estado'
        ]
        ...
        header.setSectionResizeMode(8, QHeaderView.Stretch) # Obs
        
        # Ajustar el resto al contenido
    ...
    def activar_modo_solicitudes(self):
        ...
        # Ocultar columnas: Fecha Nac (4) y Clases Restantes (7)
        self.table.setColumnHidden(4, True)
        self.table.setColumnHidden(7, True)
        
        # Renombrar cabeceras
        self.table.horizontalHeaderItem(3).setText("Teléfono 1")
        self.table.horizontalHeaderItem(6).setText("Horario")
    ...
    def agregar_fila(self, alumno):
        ...
        # Mapeo a columnas de la tabla (Siempre 10 columnas)
        # 0:ID, 1:Nombre, 2:Edad, 3:Tel, 4:FechaNac, 5:Prog, 6:Hora, 7:Clases, 8:Obs, 9:Estado
        datos_fila = [
            alumno[0],       # 0. ID
            nombre_completo, # 1. Nombre
            edad_str,        # 2. Edad
            alumno[4],       # 3. Teléfono (Tel 1)
            alumno[6],       # 4. Fecha Nac (Se oculta en modo solicitudes)
            alumno[7],       # 5. Programa
            alumno[8],       # 6. Horario
            alumno[9],       # 7. Clases Restantes (Se oculta en modo solicitudes)
            alumno[10],      # 8. Observaciones
            alumno[11]       # 9. Estado
        ]
        ...
        for col, val in enumerate(datos_fila):
            ...
            # Bloqueo de edición (excepto Estado que es la columna 9)
