    ...
    def _on_programa_selected(self, index):
        ...
        try:
            ...
            for id_clase, hora_inicio, hora_fin, capacidad, dias, id_prog_fk, nombre_maestro in filas:
                alumnos_inscritos_count = self.modelo.contar_alumnos_en_clase(id_clase) or 0
                
                cupo_disp = (capacidad - alumnos_inscritos_count) if capacidad is not None else 'N/A'
                if isinstance(cupo_disp, int) and cupo_disp < 0:
                    cupo_disp = 0

                # Obtener nombres para la lista visual
                alumnos_data = self.modelo.buscar_alumnos(id_clase=id_clase, estado='Activo')
            ...
            if ok and nombre_seleccionado:
                ...
                try:
                    if self.modelo.asignar_instructor_a_clase(id_clase, id_maestro_seleccionado):
                        ...
                        item = self.tabla_clases.item(row, INSTRUCTOR_COLUMN)
                        item.setText(nuevo_texto)
                        if nuevo_texto == "No Asignado":
                            item.setForeground(QColor("#E57373"))
                        else:
                            item.setForeground(QColor("#333333"))
                    else:
                        QMessageBox.warning(self, "Error", "No se pudo asignar el instructor.")
                except Exception as e:
